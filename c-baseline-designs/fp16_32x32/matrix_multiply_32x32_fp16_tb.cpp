/*
 * matrix_multiply_32x32_fp16_tb.cpp
 * Testbench for full 32x32 matrix multiplication using ap_fixed<16,6> precision
 */
...
int main() {
    ...
    printf("Starting full 32x32 matrix multiplication tests (ap_fixed<16,6>)...\n");
    ...
    // Initialize A with all zeros, B with rows [1, 2, 3, ..., 32]
    for (int i = 0; i < 32; i++) {
        for (int j = 0; j < 32; j++) {
            A[i][j] = 0.0;
            B[i][j] = (j + 1);  // Each row is [1, 2, 3, ..., 32]
            C[i][j] = 0.0;
        }
    }
    
    // Run the computation
    matrix_multiply_32x32_fp16(A, B, C);
    ...
    // Initialize A with all 2s, B with rows [1, 2, 3, ..., 32]
    for (int i = 0; i < 32; i++) {
        for (int j = 0; j < 32; j++) {
            A[i][j] = 2.0;
            B[i][j] = j * 0.333;  // Each row is [0, 0.333, 0.666, 0.999, ..., 10.323] (1-32 was causing bit flip error)
            C[i][j] = 0.0;
        }
    }
    ...
    // Expected result: C[i][j] = 2 × 32 × (j+1) = 64 × (j+1)
    // C = [64, 128, 192, 256, ..., 2048] for each row
    printf("Result:\n");
    for (int i = 0; i < 32; i++) {
        printf("Row %d:", i);
        for (int j = 0; j < 32; j++) {
            double c_val = C[i][j].to_double();
            double expected = 64.0 * (j * 0.333);
            printf(" %6.2f", c_val);
            result_file << c_val << " ";
