# 1 "../../library/int8_8x8/int8_8x8_wrapper.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 376 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "../../library/int8_8x8/int8_8x8_wrapper.cpp" 2
...
# 2 "../../library/int8_8x8/int8_8x8_wrapper.cpp" 2
...
# 3 "../../library/int8_8x8/int8_8x8_wrapper.cpp" 2
...
#pragma HLS interface ap_ctrl_none port=return
 ...
 ap_uint<64> a_data[8];
    VITIS_LOOP_19_1: for(int i = 0; i < 8; i++) {
#pragma HLS unroll
 a_data[i] = a_stream.read();
    ...
    ap_uint<64> b_data[8];
    VITIS_LOOP_27_2: for(int i = 0; i < 8; i++) {
#pragma HLS unroll
 b_data[i] = b_stream.read();
    ...
    VITIS_LOOP_38_3: for(int i = 0; i < 8; i++) {
        VITIS_LOOP_39_4: for(int j = 0; j < 8; j++) {
            a_mat[i][j] = a_data[i]((j+1)*8-1, j*8);
        }
    }


    VITIS_LOOP_45_5: for(int i = 0; i < 8; i++) {
        VITIS_LOOP_46_6: for(int j = 0; j < 8; j++) {
    ...
    VITIS_LOOP_53_7: for(int i = 0; i < 8; i++) {
        VITIS_LOOP_54_8: for(int j = 0; j < 8; j++) {
            VITIS_LOOP_55_9: for(int k = 0; k < 8; k++) {
    ...
    VITIS_LOOP_62_10: for(int i = 0; i < 8; i++) {
#pragma HLS unroll
 ap_uint<128> c_row = 0;
        VITIS_LOOP_65_11: for(int j = 0; j < 8; j++) {
#pragma HLS unroll
 ap_int<16> val = c_mat[i][j];
            VITIS_LOOP_68_12: for(int b = 0; b < 16; b++) {
#pragma HLS unroll
 c_row[j*16 + b] = val[b];
            }
        }
