...
module int8_32x32_wrapper(
    ...
    reg [255:0] a_buffer [0:31];  // Buffer 32 rows of A (256-bit each)
    reg [255:0] b_buffer [0:31];  // Buffer 32 columns of B (256-bit each)
    reg [511:0] c_buffer [0:31]; // Buffer 32 result rows (512-bit each)
    ...
    // Compute operation counter for orchestrating 256 slice operations (8x8 per slice × 16 slices)
    reg [8:0] operation_count;
    reg [4:0] row_idx, col_idx;  // Which 8x8 block we're processing (0-3 each)
    
    // Current row and column inputs to tensor slices
    reg [63:0] a_row [0:3];  // a_row[i] for row block i
    reg [63:0] b_col [0:3];  // b_col[j] for column block j
    ...
    always @(posedge ap_clk) begin
        if (ap_rst) begin
            ...
            case (state)
                ...
                // Compute: orchestrate 16 tensor slices for all 256 operations (4x4 blocks × 8x8 each)
                COMPUTE: begin
                    if (&done_slice) begin  // All 16 slices done
                        operation_count <= operation_count + 1;
                        
                        // Aggregate results into c_buffer
                        // For 4x4 grid computing 8x8 blocks:
                        // Extract and pack results from c_out[] into c_buffer[]
                        // Each block produces 8 rows of 8 int16 values (128 bits)
                        
                        if (operation_count == 255) begin
                            state <= SEND_C;
                            c_row_count <= 6'd0;
                        end
                        
                        // Increment indices for next operation
                        if (col_idx == 5'd31) begin
                            col_idx <= 5'd0;
                            row_idx <= row_idx + 1;
                        end else begin
                            col_idx <= col_idx + 1;
                        ...
                        a_row[2] <= a_buffer[row_idx + 16];
                        a_row[3] <= a_buffer[row_idx + 24];
                        
                        b_col[0] <= b_buffer[col_idx];
                        b_col[1] <= b_buffer[col_idx + 8];
                        b_col[2] <= b_buffer[col_idx + 16];
                        b_col[3] <= b_buffer[col_idx + 24];
                    end
                end
    ...
    assign start_slice = {16{(state == COMPUTE && operation_count < 256)}};
