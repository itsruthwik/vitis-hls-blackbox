/*
 * matrix_multiply_128x128_32api.cpp
 * Top-level HLS function for 128x128 matrix multiplication
 * Uses 4x4 tiling of int8_32x32_wrapper blackbox for computation
 */

#include "matrix_multiply_128x128_32api.h"

// Top-level function for 128x128 matrix multiplication using 32x32 tiles
...
)
{
    ...
    ap_int<8> a_local[128][128];
    ap_int<8> b_local[128][128];
    ap_int<16> c_local[128][128];
...
#pragma HLS PIPELINE II=1
    ...
    // Perform tiled multiplication: 4x4 tiling with k accumulation
    for(int ti = 0; ti < 4; ti++) {          // Output row tiles
        for(int tj = 0; tj < 4; tj++) {      // Output col tiles
            for(int tk = 0; tk < 4; tk++) {  // Inner dimension tiles
