/*
 * matrix_multiply_64x64_32api_tb.cpp
 * Testbench for 64x64 matrix multiplication using 32x32 wrapper tiles
 */
...
int main() {
    ...
    // Test 1: 0 x [1 2 3 ... 64]
    printf("Starting 64x64 matrix multiplication tests (using 32x32 API tiles)...\n");
    printf("Test 1: 64x64 matrix multiplication with 0 x [1,2,3,...,64]\n");
    ...
    // Run the computation
    matrix_multiply_64x64_32api(A, B, C);
    ...
    // Test 2: 2 x [1 2 3 ... 64]
    printf("\nTest 2: 64x64 matrix multiplication with 2 x [1,2,3,...,64]\n");
    for (int i = 0; i < 64; i++) {
        for (int j = 0; j < 64; j++) {
            A[i][j] = 2;
            B[i][j] = j + 1;
    ...
    // Expected result: each C[i][j] = 128 * (j+1)
    for (int i = 0; i < 64; i++) {
        printf("Row %d:", i);
        for (int j = 0; j < 64; j++) {
            printf(" 0x%04x", (unsigned short)C[i][j]);
